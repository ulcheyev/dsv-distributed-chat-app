...
public class DsvRequestsQueue {
    ...
    public synchronized <ReqT, RespT> CompletableFuture<ServerCall.Listener> handle(ServerCallHandler<ReqT, RespT> serverCallHandler, ServerCall<ReqT, RespT> serverCall, Metadata metadata) {
        ...
        if (isNodeInCS() || working) {
            if (canHandleNow(serverCall.getMethodDescriptor())) {
                ...
                logger.log(Level.INFO, "Request {0} is delayed", serverCall.getMethodDescriptor().getBareMethodName());
    ...
    private static synchronized void processRequestInThread(DsvPair<ServerCallHandler, DsvPair<ServerCall, Metadata>> req, CompletableFuture<ServerCall.Listener> future) {
        ex.submit(() -> {
            try {
                logger.log(Level.INFO, "Processing request: {0}", req.getValue().getKey().getMethodDescriptor().getBareMethodName());
                working = true;
    ...
    public static synchronized void processNextRequest() {
        if (!requests.isEmpty()) {
            ...
            if (!working) {
                ...
                logger.log(Level.INFO, "Request {0} is delayed", peeked.getValue().getValue().getKey().getMethodDescriptor().getBareMethodName());
            }
        }
    }
